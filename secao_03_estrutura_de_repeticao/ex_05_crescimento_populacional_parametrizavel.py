...
def calcular_ano_ultrapassagem_populacional(
    ...
    if taxa_crescimento_populacao_maior < taxa_crescimento_populacao_menor:
        ...
        while populacao_menor < populacao_maior:
           ...
           tempo+=1
        print(f"'População de A, depois de {tempo} ano(s) será de {populacao_menor:.0f} pessoas, superando a de B, que será de {populacao_maior:.0f} pessoas'")
    else:
        print (f"'A taxa de crescimento do país B ({taxa_crescimento_populacao_maior:.1%}) deve ser menor do que a do país A ({taxa_crescimento_populacao_menor:.1%})'")